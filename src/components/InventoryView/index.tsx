...
export function InventoryView({ 
  ...
  const handleSave = async (formData: Omit<Product, 'id' | 'createdAt' | 'updatedAt'>) => {
  ...
  const filteredProducts = products.filter(product =>
    searchQuery ? (
      ...
      suppliers.find(s => s.id === product.supplierId)?.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b">
              ...
              <th className="text-left py-4">{t('inventory.productDescription')}</th>
